...
--
-- Modification to allow the use of spaces in table and field names
-- by using quoted identifier delimiters to wrap table and field names
--
-- Added delimiters for the four tested databases
-- Uncomment the desired delimiter to match the database you are using
-- NOTE: SQLite respects all DB delimiter types -- potentially useful for testing
-- 

-- DS = D(elimiter)S(tart) & DE = D(elimiter)E(nd)
-- default to no delimiter = same behaviour as original code
local DS ='' DE = ''
...
local function addRowInsert(TableName, Row, Buffer)   
   ...
   for i=1, #Row do
      if Row[i]:nodeValue() ~= '' or
         ...
         Buffer:write(DS..Row[i]:nodeName()..DE)   
         NotFirstVal = true
      end
   end   
   Buffer:write(') VALUES(')
...
local function genericMergeSQL(Table, Statements, Keys)
   for i=1, #Table do
      ...
      if count == 0 then
         addRowInsert(DS..Table:nodeName()..DE, Table[i], Buffer)
      elseif count == 1 then
         addRowUpdate(DS..Table:nodeName()..DE, Table[i], Buffer, Keys)         
      else         
